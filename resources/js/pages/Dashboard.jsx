...
const HERO_BACKGROUNDS = [
  {
    ...
    glowA: "bg-cyan-400/35",
    glowB: "bg-fuchsia-400/35",
    glowC: "bg-violet-400/35",
  },
  {
    bg: "bg-[linear-gradient(135deg,#0b1320_0%,#10243d_38%,#064e3b_100%)]",
    glowA: "bg-emerald-400/35",
    glowB: "bg-cyan-400/35",
    glowC: "bg-teal-400/35",
  },
  {
    bg: "bg-[linear-gradient(135deg,#140f1f_0%,#1e1b4b_36%,#7c2d12_100%)]",
    glowA: "bg-amber-400/35",
    glowB: "bg-fuchsia-400/35",
    glowC: "bg-orange-400/35",
  },
  {
    bg: "bg-[linear-gradient(135deg,#08111f_0%,#172554_36%,#4a044e_100%)]",
    glowA: "bg-sky-400/35",
    glowB: "bg-violet-400/35",
    glowC: "bg-pink-400/35",
  },
];
...
export default function Dashboard({
  ...
  const metrics = useMemo(() => {
    ...
    const validacion = ventas.filter((v) => v.estado === "Validación").length;
    const tramitadas = ventas.filter((v) => v.estado === "Tramitada").length;
    const activadas = ventas.filter((v) => v.estado === "Activada").length;
    const rechazadas = ventas.filter((v) => v.estado === "Rechazada").length;

    const leadsPendientes = leads.filter((l) => l.estado === "Pendiente").length;
    const leadsContactados = leads.filter((l) => l.estado === "Contactado").length;
    const leadsRellamada = leads.filter((l) => l.estado === "Rellamada").length;
  ...
  const monthlyTrend = useMemo(() => {
    ...
    ventas.forEach((v) => {
      ...
      if (v.estado === "Activada") target.activadas += 1;
    });
  ...
  const weeklyTrend = useMemo(() => {
    ...
    ventas.forEach((v) => {
      ...
      target.ventas += 1;
      if (["Tramitada", "Activada"].includes(v.estado)) {
        target.cierres += 1;
      }
    });
    ...
    days.forEach((day) => {
      leads.forEach((l) => {
        const d = safeDate(l.fecha || l.createdAt);
        if (!d) return;
        const key = d.toISOString().slice(0, 10);
        if (day.key === key) {
          day.leads += 1;
  ...
  return (
    <div className="space-y-6">
      <div
        className={`relative overflow-hidden rounded-[34px] border border-white/10 p-5 text-white shadow-[0_30px_90px_rgba(6,11,20,0.30)] transition-all duration-1000 ${heroTheme.bg}`}
      >
        <div className={`pointer-events-none absolute -left-16 top-0 h-52 w-52 rounded-full blur-3xl transition-all duration-1000 ${heroTheme.glowA}`} />
        <div className={`pointer-events-none absolute right-0 top-0 h-52 w-52 rounded-full blur-3xl transition-all duration-1000 ${heroTheme.glowB}`} />
        <div className={`pointer-events-none absolute bottom-0 left-[35%] h-48 w-48 rounded-full blur-3xl transition-all duration-1000 ${heroTheme.glowC}`} />
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            {executiveStrip.map((item) => {
              ...
              return (
                <div
                  key={item.label}
                  className="rounded-[24px] border border-white/10 bg-white/10 px-4 py-4 backdrop-blur-md"
                >
                  <div className="mb-3 flex items-center justify-between gap-3">
                    <p className="text-xs font-medium uppercase tracking-[0.20em] text-slate-200">
                      {item.label}
                    </p>
                    <div
      ...
      <div className="grid gap-6 xl:grid-cols-[1.25fr_0.75fr]">
        ...
        <div className={glowCardClass()}>
          <div className="mb-5 flex items-center gap-3">
            ...
            <h3 className="text-lg font-semibold text-white">Estado de ventas</h3>
          </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_1fr]">
        <div className={glowCardClass()}>
          ...
          <div className="h-[320px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={topCampañas} layout="vertical" margin={{ left: 20 }}>
        ...
        <div className={glowCardClass()}>
          <div className="mb-5 flex items-center gap-3">
            ...
            <h3 className="text-lg font-semibold text-white">Estado de leads</h3>
          </div>
      ...
      <div className="grid gap-6 xl:grid-cols-3">
        <div className={glowCardClass()}>
          ...
          <p className="text-4xl font-bold text-white">{metrics.pendientes}</p>
        ...
        <div className={glowCardClass()}>
          <div className="mb-4 flex items-center gap-3">
            ...
            <h3 className="text-lg font-semibold text-white">Tramitadas + activadas</h3>
          </div>
        ...
        <div className={glowCardClass()}>
          <div className="mb-4 flex items-center gap-3">
            ...
            <h3 className="text-lg font-semibold text-white">Rechazadas</h3>
          </div>
          <p className="text-4xl font-bold text-white">{metrics.rechazadas}</p>
          <p className="mt-3 text-sm leading-7 text-slate-300">
