...
export default function Ventas({ ventas = [], setVentas, currentUser }) {
  ...
  const campañasDisponibles = useMemo(() => {
    return ["Todas", ...new Set(ventas.map((v) => v.campana).filter(Boolean))];
  }, [ventas]);
  ...
  useEffect(() => {
    if (selectedVenta) {
      setEditForm({
        ...
        documento: selectedVenta.documento || "",
        telefono: selectedVenta.telefono || "",
        campana: selectedVenta.campana || "",
        comercial: selectedVenta.comercial || "",
        coordinador: selectedVenta.coordinador || "",
        supervisor: selectedVenta.supervisor || "",
        producto: selectedVenta.producto || "",
        estado: selectedVenta.estado || "",
        fecha: selectedVenta.fecha || "",
        hora: selectedVenta.hora || "",
        ficha: cleanFichaObject(selectedVenta.ficha || {}),
      });
    }
  ...
  const cambiarEstado = (nuevoEstado) => {
    ...
    setVentas((prev) =>
      prev.map((venta) =>
        venta.id === selectedVenta.id ? { ...venta, estado: nuevoEstado } : venta
      )
    );
  };
  ...
  const exportarExcel = () => {
    const data = ventas.map((venta) => flattenVentaForExport(venta));
  ...
  const exportarPDF = () => {
    ...
    ventas.forEach((venta, index) => {
      ...
      if (currentY > 260 && index < ventas.length - 1) {
        doc.addPage();
        currentY = 14;
      }
    });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="crm-panel p-5">
          <div className="flex items-center gap-3">
            ...
            <p className="crm-label">Tramitadas</p>
          </div>
        ...
        <div className="crm-panel p-5">
          <div className="flex items-center gap-3">
            ...
            <p className="crm-label">Rechazadas</p>
          </div>
      ...
      <div className="crm-panel p-5">
        <div className={`grid gap-4 ${canSeeExportButtons ? "xl:grid-cols-[1.2fr_220px_220px_auto_auto]" : "xl:grid-cols-[1.2fr_220px_220px]"}`}>
              ...
              {editMode && canEditVentas ? (
                ...
                <>
                  ...
                  <div className="crm-panel-soft p-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {["Pendiente", "Validación", "Tramitada", "Rechazada", "Activada"].map(
                        (estado) => (
                          <button
                            key={estado}
